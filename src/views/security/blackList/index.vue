  ...
  <div class="black-list">
    <el-form
      ...
      <el-form-item v-if="btnAuthority($route.path, 0)">
        <bx-button class="addBtnSecurity" type="text" @click="handlerAdd" label="添加黑名单安全组"></bx-button>
      </el-form-item>
    ...
    <el-table
      ...
      stripe>
      <el-table-column prop="blackGroupName" label="安全组名称" align="center" min-width="200"></el-table-column>
      <el-table-column label="黑名单IP" align="center" min-width="750">
        <template slot-scope="scope">
          <el-popover
            ...
            width="700"
            trigger="hover"
            :content="scope.row.blackGroupIps"
          >
            <div style="width: 100%" slot="reference">
              <span class="ellipsis-span">{{scope.row.blackGroupIps}}</span>
            </div>
          </el-popover>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <i
            ...
            @click="handlerEdit(scope.row.blackGroupId)"
            v-if="btnAuthority($route.path, 1)"
          ></i>
          <i
            class="iconfont iconbutton_delete act-img-btn"
            @click="handlerDelete(scope.row.blackGroupId)"
    ...
    <el-dialog
      ...
      <el-menu
        ...
        <div class="add" v-if="addClick">
          <el-menu-item index="1">增加黑名单安全组</el-menu-item>
        </div>

        <div class="edit" v-else>
          <el-menu-item index="1">修改黑名单组</el-menu-item>
          <el-menu-item index="2" v-if="btnAuthority($route.path, 3)">黑名单分配</el-menu-item>
        </div>
      </el-menu>
      ...
      <div class="add-list" v-if="activeIndex == 1">
        <el-form
          ...
          :model="blackListForm"
        >
          <el-form-item label="安全组组名" prop="blackGroupName">
            <el-input v-model="blackListForm.blackGroupName" maxlength="20"></el-input>
          </el-form-item>

          <el-row>
            <el-col :span="24">
              <el-form-item label="黑名单IP" prop="blackGroupIps">
                <el-input
                  v-model="blackListForm.blackGroupIps"
                  type="textarea"
                  :autosize="{ minRows: 2, maxRows: 4}"
                  placeholder="请输入黑名单IP，多个IP请用 '，' 隔开"
                ></el-input>
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <div slot="footer" class="dialog-footer" v-if="activeIndex == 1">
        <el-button class="reset" @click="resetForm('addBlackList')">重置</el-button>
        <bx-button
          class="confirm"
          type="primary"
          @click="blackListForm.id === null ? submitAdd('addBlackList') : submitEdit('addBlackList')"
          label="确认"
        ></bx-button>
      </div>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        blackGroupIps: [{ required: true, validator: ipRoles, trigger: "blur" }]
      },
      dialogVisible: false,
      activeIndex: "1",
      addClick: false, // 添加对话框：true， 编辑对话框：false
      show: true, // 是否显示 分配人员列表
  ...
  methods: {
    ...
    submitEdit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          updateBlackGroup({
            blackGroupId: this.blackListForm.id,
            blackGroupName: this.blackListForm.blackGroupName,
            blackGroupIps: this.blackListForm.blackGroupIps
          }).then(res => {
            this.dialogVisible = false;
    ...
    getAllocationPowerData() {
      getAllocationPowerDataNew(this.id, getRouterId(this.$route.path), 2).then(
        res => {
    ...
    handlerEdit(id) {
      ...
      // 回显黑名单组详情
      getBlackGroupInfo(id).then(res => {
        this.blackListForm.id = res.data.blackGroupId;
        this.blackListForm.blackGroupName = res.data.blackGroupName;
        this.blackListForm.blackGroupIps = res.data.blackGroupIps;
      });
    },
    ...
    handlerDelete(id) {
      this.$confirm("此操作将永久删除该黑名单安全组, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        delBlackGroup(id).then(res => {
          this.clearInput();
          this.fetchData();
        });
      }).catch(() => {});
